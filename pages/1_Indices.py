...
# Streamlit - header
st.header('EDA - Indices')

indices = nse.nse_get_index_list() # indices list


script = st.selectbox('Indices',indices,index=None)
...
if start_date<end_date:
    df = nse.index_history(script,start_date=str(start_date),end_date=str(end_date))
    

    df = df.set_index('HistoricalDate')
    ...
    if type and script and df.shape[0]>0:
        ...
        with st.expander('**3. Distribution**'):
            ...
            plt.axvspan(-st_d,st_d,color = 'grey',ymin=y_min,ymax=y_max,alpha = 0.5,label = f"+/- std : [{-st_d},{st_d}]")
            plt.axvspan(-st_d*2,st_d*2,color = 'grey',ymin=y_min,ymax=y_max,alpha = 0.3,label = f"+/- 2std : [{-st_d*2},{st_d*2}]")
            plt.axvspan(-st_d*3,st_d*3,color = 'grey',ymin=y_min,ymax=y_max,alpha = 0.1,label = f"+/- 3std : [{-st_d*3},{st_d*3}]")
